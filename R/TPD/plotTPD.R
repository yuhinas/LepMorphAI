...
# parameters for dimensions > 2
# assign plotAxisX, plotAxisX for the axes to plot
plotTPD<- function(TPD, whichPlot = NULL, nRowCol = NULL, color1 = "grey60",
                   leg = TRUE, leg.text = NULL, leg.pos = "topright", leg.cex = 1, 
                   plotAxisX=1, plotAxisY=2, alignLimXY = T, probColors=NULL) {
  ...
  if (dimensions > 2){
    print("plotTPD only works for 1 or 2 dimensional trait spaces.")
  }
  ...
  if (dimensions >= 2) {
    ...
    limx <- range(TPD_grid[, plotAxisX])
    limy <- range(TPD_grid[, plotAxisY])

    if (alignLimXY) {
      limxy <- range(limx, limy)
      limx <- limxy
      limy <- limxy
    }
    ...
    if (is.null(probColors)) {
      probFill = '#132b43'
    } else {
      probColors = c(probColors)
      probColorsLen = length(probColors)
    }
    ...
    for (pli in 1:length(whichPlot)) {
      ...
      mat_ <- cbind(TPD_grid[,c(plotAxisX,plotAxisY)], TPD_probs[[pli]])
      names(mat_)<-c("T1", "T2", "prob")

      # aggregate probabilities to axes being displayed
      if (dimensions > 2) {
        mat = data.table(mat_)[, .(prob=sum(prob)), by=c('T1', 'T2')]
      } else {
        mat = mat_
      }

      probColorIdx = pli %% probColorsLen
      if (probColorIdx == 0)
        probColorIdx = probColorsLen
      probFill = probColors[probColorIdx]
      mat$prob[mat$prob == 0] <- NA
      pl[[pli]] <- with(mat, ggplot2::ggplot(mat, ggplot2::aes(T1, T2,  fill = prob),
                                             interpolate=T)) +
        ggplot2::geom_raster() +
        ggplot2::scale_fill_gradient(low=probFill, high='white', na.value = "grey90") +
...
## example
## plotTPD(TPDc_imp, plotAxisX = 2, plotAxisY = 3, probColors = c('dodgerblue4', 'firebrick3'), leg.text = c('Before 100MA', 'Aftere 100MA'))